...
| `description`                 | String  | `"§aVanilla Survival §c[1.20.4]"` | The Server MOTD (Message of the Day). Supports standard Minecraft color codes (`§`).                               | Any string.                                                                                                                |
...
| `protocol_behavior`           | String  | `"Chameleon"`                     | Determines how the server responds to different client protocol versions.                                          | `"Chameleon"` (mimics client), `"Fixed"` (uses static version), `"Random"` (generates random protocol every time between). |
...
| `min_random_protocol_version` | Integer | `1`                               | The minimum protocol version number to send back when `protocol_behavior` is set to `"Random"`.                    | Any integer representing a Minecraft protocol version.                                                                     |
| `max_random_protocol_version` | Integer | `776`                             | The maximum protocol version number to send back when `protocol_behavior` is set to `"Random"`.                    | Any integer representing a Minecraft protocol version.                                                                     |
...
| `trap`                        | Object  | (nested object)                   | Configuration section for honeypot trap, fake players generation, and tarpit mechanisms.                           | JSON Object.                                                                                                               |
